...
using TooService.Entities;

namespace TooService.Data
{
        ...
        where T : IEntity
    {
        ...
        public bool Update(FilterDefinition<T> filter, params UpdateDefinition<T>[] updates)
        {
            return Retry(() =>
            {
                var update = Updater.Combine(updates).CurrentDate(i => i.UpdatedOn);
                return Collection.UpdateMany(filter, update.CurrentDate(i => i.UpdatedOn)).IsAcknowledged;
            });
        }
